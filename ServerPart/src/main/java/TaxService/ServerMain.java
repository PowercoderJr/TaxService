...
public class ServerMain
{
	...
	private static void dropNcreate(Connection connection) throws SQLException
	{
		try (Statement stmt = connection.createStatement())
		{
			...
			//Создание таблиц
			String execMe;
			execMe = "create table department(" + "id serial primary key," + "deptype_id int8 not null references deptype(id) on delete cascade," + "name varchar(100) not null," + "startyear numeric(4,0) not null," + "phone varchar(17) not null," + "city_id int8 not null references city(id) on delete cascade," + "street varchar(30) not null," + "house varchar(6) not null)";
			stmt.executeUpdate(execMe);

			execMe = "create table employee(" + "id serial primary key," + "surname varchar(30) not null," + "name varchar(30) not null," + "patronymic varchar(30) not null," + "department_id int8 not null references department(id) on delete cascade," + "birthdate date not null," + "post_id int8 not null references post(id)," + "salary int4 not null," + "education_id int8 not null references education(id) on delete cascade)";
			stmt.executeUpdate(execMe);

			execMe = "create table company(" + "id serial primary key," + "name varchar(100) not null," + "owntype_id serial not null references owntype(id) on delete cascade," + "phone varchar(17) not null," + "startyear numeric(4,0) not null," + "statesize int4 not null)";
			stmt.executeUpdate(execMe);

			execMe = "create table payment(" + "id serial primary key," + "paytype_id int8 not null references paytype(id) on delete cascade," + "date date not null," + "amount numeric(12,2) not null," + "employee_id int8 not null references employee(id) on delete cascade," + "department_id int8 not null references department(id) on delete cascade," + "company_id int8 not null references company(id) on delete cascade)";
			stmt.executeUpdate(execMe);
